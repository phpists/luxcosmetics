        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="header__container">
                    <div class="header__logo"><a href="{{route('home')}}"><img src="{{asset('images/dist/logo.svg')}}"
                                                                               alt=""></a></div>
                    <form action="{{route('show_search')}}" class="header__search search" id="search_field_header">
                        <input type="text" minlength="2" name="search" id="header_search" class="search__input"
                               placeholder="Поиск по каталогу">
                        <div id="header-container" style="position: relative">
                            <div id="search_results"
                                 style="position: absolute; background: whitesmoke; z-index: 2; width: 100%; display: none"></div>
                        </div>
                    ...
                    <div class="header__contacts">
                        ...
                        @foreach (\App\Models\SocialMedia::phone()->get() as $item)
                            @if($item->phone)
                                <div class="header__phone"><a
                                        href="tel:{{ preg_replace('/[^0-9.]/', '', $item->phone) }}">{{ $item->phone }}</a>
                                </div>
                            @endif
                        @endforeach
                        <div class="header__links">
                            ...
                            <a href="{{route('favourites')}}" class="header__link header__link--fav">
                                ...
                                <span
                                    class="header__linkcount @if(sizeof(\App\Services\FavoriteProductsService::getAllIds()) === 0) hidden @endif"
                                    id="header__linkcount">{{sizeof(\App\Services\FavoriteProductsService::getAllIds())}}</span></a>
                            <a href="{{route('cart')}}" class="header__link header__link--cart">
                                <svg class="icon">
                                    <use xlink:href="{{asset('images/dist/sprite.svg#cart')}}"></use>
                                </svg>
                                <span class="header__linkcount"
                                      id="cartTotalCount">{{ $cartService->getTotalCount() }}</span></a>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    @foreach($menu_items->whereNull('parent_id')->where('type', \App\Models\Menu::TOP_MENU) as $menu_item)
                        <li>
                            @if(sizeof($menu_item->getChildren($menu_items)) > 0)
                                <a class="@if($current_url == $menu_item->link) active @endif"
                                                ...
                                                <div class="submenu__wrapper">
                                                    <ul class="submenu__menu">
                                                        @foreach($menu_item->getChildren($menu_items) as $submenu)
                                                            <li>
                                                                <a class="@if(str_contains($current_url, $submenu->link)) active @endif"
