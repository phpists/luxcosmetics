...
class FilterBak extends Component
{
    ...
    public function getFilters()
    {
        ...
        $filterWeights = $this->filterWeights = $this->getFiltersWeight($filters, $this->allProducts);
        $filters = $filters->filter(function ($filter) use($filterWeights) {
            return $filter->values->filter(function ($filter_value) use ($filter, $filterWeights) {
                return $filterWeights[$filter->id][$filter_value->id] > 0;
            })->isNotEmpty();
        });

        $filters->each(function($property) use($filterWeights) {
            $property->values = $property->values->filter(function ($filter_value) use ($property, $filterWeights) {
                return $filterWeights[$property->id][$filter_value->id] > 0;
            });
